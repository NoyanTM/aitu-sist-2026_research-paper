...
% subpart: regulations, hidden malware (Ken Thompson's hack), distinction between design and implementation
...
% --------------------------------------------------------------------------
Futhermore, проведении compromise assessment and software evaluation такого полученного product будут затруднено, ведь зараженные reverse engineering and deconstruction tools (e.g., debugger, decompiler, dissassembler) или их чистые версии но уже запущенные в compromised environment будут blinded in some cases to hide зараженную parts and backdoors в ПО in order to be remained undetected. Поэтому нужно тщательно понимать кому доверять и по каким принципам, чтобы предварительно минимизировать соотвественные риски. Исследователи Singer and Bishop предлагают рассматривать trust в форме relationship модели делающий нас vulnerable из-за того что мы полагаемся и надеемся на third-party без всяких сомнений (считаем их trustworthy) \cite{singerTrustBasedSecurityTrust2020}. Они также советуют применять моделирование угроз и анализ рисков перед и во время design стадией для минимизации фактора of “trust”. Это ведет к тому что trust следует рассматривать как наши недочеты и assumption when мы are distracted доверлись какой-нибудь системе. Скажем что есть интернет протоколы и способы передачи данных где получатель и отправитель будут соответствовать стандартам / интерфейсу взаимодействия то данные будут передаваться в корректной форме. Но ничто не мешает одной из сторон disrupt такое положение в любой момент. Trust это не статичный и точный показатель, это больше как probability of someone being more trusted and being trusted by someone else (процент доверия других к нему и то что он не обманет с определенной вероятностью других) (и trust очень дорогое - любая ошибка ведет к ее моментальной потере с обоих сторон). % trusted/semi-trusted/untrusted

% subpart: zero trust as theory and in blockchain
В свою очередь, National Institute of Standards and Technology (NIST) сформировал framework как the Zero Trust Architecture (ZTA), где trust requires continues verification \cite{roseZeroTrustArchitecture2020}. Babenko et al. clearly demonstrated that empirical data on ZTA implementation correlates with a lower incident rate of insider threats \cite{babenkoCybersecuritylevelAssessmentModels}. Organizations могут adapt that следуя перечисленным основным принципам:
\begin{itemize}
    \item Continually evaluation всех действий и акторов вместо полного доступа внутри защищенного периметра. То есть сам environment does not affect на то как мы trust to someone/something. (giving assumption that we operate in hostile environment everytime)
    \item Избавляемся от implicit trust на explicit one за счет trust algorithm как способ проверки доступа или credability с transactional properties (где если не прошел хотя бы один из этапов то не можешь считаться trusted). % TODO: cite zerotrust original document that page
    \item Сегментация сети и систем на atomic ячейки, что теоретически дробит attack surface (e.g., large monolith will expose more data than microservices в отдельности) \cite{JourneyZeroTrust}.
    \item strict comprehensive assessments of vendors, suppliers, and corresponding software supply chains, as any characteristics in external dependencies can impact whole IT infrastructure. “minimizing access то есть снижение кол-ва зависимостей (and least privileges) и тщательная их проверка перед тем как одно хотя бы добавить/разрешить. \cite{collierZeroTrustSupply2021}. % TODO: add reference leeDoDEnterpriseDevSecOps
    \item Security as dynamic process and not static one (каждый раз меняется, анализирует, и адаптируется под изменения). security as dynamic and adaptive system, attack vectors (add reference pourmoafiSolutionSecuringInformation and mengCybersecuritySimondonsConcretization2022). Постоянный сбор и анализ различных данных об имеющихся ресурсах, активах, инцидентах, и другое - также референс на explainable AI и контекст анализа данных в ZTA (mushtaqSystematicLiteratureReview2025)
\end{itemize}
Одним из примеров реальной имплементации ZTA может быть блокчейн системы, где присутствует zero knowledge proofs and consensus algorithms such as proof-of-x гарантирующие доверее на основе определенного trust фактора (вычислительная мощность, кол-во устройств, или кол-во определенных токенов) \cite{banguiWhenTrustlessMeets2024}. Помимо этого в той сфере есть такие концепции "trustless" которые еще сильнее усиливают ZTA and trust management подходы в распределенных системах. Некоторые исследователи вовсе рассматривают потенциал применение блокчейн как version control систему из-за data integrity and non-repudation характеристик \cite{grilliCombiningGitBlockchain2024}.

% subpart: secure by design and rethinking software ecosystems
Overall, индустрии необходимо замедлится и переосмыслить то что уже было произведено, остановив перепроизводство software and start recycling waste (old and заброшенные идеи) переосмысливая их, находя закономерности и повторения. Это связано с тем, что очень высокая фрагментация из решений которые с друг другом конкурируют, но в корне они решают одну и ту же задачу (e.g., frontend frameworks) \cite{enablingGlobalCollaboration}. Mirakhorli et al. reviewed 84 available SBOM management toolkits across multiple programming language ecosystems, which demonstrates the lack of unification and significant fragmentation in current solutions \cite{mirakhorliLandscapeStudyOpen2024}. Разработчики должны стремится наоборот регулярно улучшать standard library и делают ее better with batteries included в официальной поставке, иначе на ее место появляется множество конкурирующих dependencies на одну определенную тему. Помимо этого эксперты предлагают делать системы таким образом чтобы они были secure by design in their foundation где безопасность встраиваться изначально а не дополнительный элемент \begin{comment} TODO: add reference "secure by demand\end{comment}. Это может гарантировать elemination specific categories of vulnerabilities (не все конечно а лишь часть), сформировав процесс of multilayered security and defense in depth \cite{mellMeasuringImprovingEffectiveness2016}. Также системы должны быть более комплексными до такой степени чтобы они могли самоанализировать себя и доулучшать as next-generation systems (that can analyze themselves) и делать системы более надежными (less fragile). К примеру, Berhe et al. showed the necessity for a mitigation plan and a regular budget on automated dependency recovery or manual actions such as replacing it with alternatives, upgrading it, or reverting to a previous release \cite{berheMaintenanceCostSoftware2023}. Если мы возьмем well-decomposed systems might still continue to function in such cases, or have recovery mechanisms ("self-healing") for the failure of individual components only up to a certain limit, after which the entire system becomes non-operational or some functionality is temporarily disabled \cite{dehrajReviewArchitectureModels2021}.
% -----------------------------------------------------------------------------
